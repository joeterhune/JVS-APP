...
$query = "
    select
		...
		CASE 
			...
			ELSE 0
		END AS isOrder,
		DATE_FORMAT(deletion_date,'%m/%d/%Y') as DeletionDate
    from
        workflow
    where
        queue = :queue
...
$deleted = array();
getData($deleted, $query, $dbh, array('queue' => $queue));

if($pagesize == "All"){
	$pagesize = count($deleted);
}
...
if($pagenum == 0){
	...
	if($endrow > count($deleted)){
		$endrow = count($deleted);
	}
}
...
$smarty->assign('lastpage', ceil(count($deleted) / $pagesize));
$smarty->assign('rowCount', count($deleted));
...
$html = $smarty->fetch('workflow/showDeleted.tpl');
...
if(!$ajax){
	...
	$headers = array("UCN", "case_style", "Title", "creator", "CreationDate", "DeletionDate", "Comments", "ReturnItemtoQueue");
	...
	$headerInfo[5]['name'] = "Deletion Date";
	$headerInfo[5]['type'] = "S";
	$headerInfo[5]['colname'] = "DeletionDate";
	$headerInfo[5]['filter_placeholder'] = "Deletion Date";
	...
	$headerInfo[7]['name'] = "Return Item to Queue";
	$headerInfo[7]['type'] = "S";
	$headerInfo[7]['colname'] = "ReturnItemtoQueue";
	$headerInfo[7]['filter_placeholder'] = "";
	$headerInfo[7]['filter_type'] = "filter-disabled";
	...
	if(empty($_REQUEST['filter'])){
		$dataArray['total_rows'] = count($deleted);
	}
	else{
