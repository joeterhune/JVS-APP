...
createTab("HON. " . $name, "/case/judge.php?val=" . $val, 1, 1, "cases");

$smarty->assign('judgeName', $name);
...
foreach ($divarr as $adiv) {
	...
	if (file_exists($civpath)) {
		$path=$civpath;
        $divInfo['divType'] = "civ";
	} else if  (file_exists($crimpath)) {
		$path=$crimpath;
        $divInfo['divType'] = "crim";
	} else if  (file_exists($juvpath))  {
		$path=$juvpath;
        $divInfo['divType'] = "juv";
	} else if  (file_exists($propath))  {
		$path=$propath;
        $divInfo['divType'] = "pro";
	} else {
		$path="";
...
$html = $smarty->fetch('reports/judgeReport.tpl');
