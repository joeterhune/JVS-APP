	...
	$(document).ready(function (){
		...
		$('#finishedItems').tablesorter ({
	        ...
	        widgetOptions: {
	            ...
	            filter_reset : '#finishedItems-reset',
		        {literal}
	            ...
	            pager_ajaxUrl: '/workflow/showfinished.php?queue={$queue}&ajax=1&pagenum={literal}{page}{/literal}&{literal}{filterList:filter}{/literal}&{literal}{sortList:column}{/literal}&pagesize={literal}{size}{/literal}',
		        ...
		        pager_ajaxObject: {
		          ...
		          success : function(data) {
					...
					$("#finishedItems").find('input').placeholder();
					
					//This screws up paging...
					$('#finishedItems').trigger('sortupdate');
					$('#finishedItems').trigger("appendCache");
		...
		$(document).on('click','.unfinish',function() {
	        ...
	        $('#dialogSpan').html("Are you sure you want to return this item to the workflow queue?  It will no longer be marked as \"Finished.\"");
			$('#dialogDiv').dialog({
				resizable: false,
				minheight: 150,
				width: 500,
...
<div style="padding:1%">
	...
	<table id="finishedItems" style="width:100%">
	    <thead>
	...
	{else}
		No finished items were found for this queue.
	{/if}
</div>