...
sub buildImageFile {
	...
	if (!defined($tmpFileDir)) {
		$tmpFileDir = "$ENV{'APP_ROOT'}/tmp";
		$hadTmpDir = 0;
	}


	my ($cfgfh, $cfgfile) = tempfile (
		DIR => "$ENV{'APP_ROOT'}/tmp",
		UNLINK => 0
	);
	...
	if (((!defined($imageref)) || (scalar(@{$imageref}) > 1)) && (!$withEnv)) {
		...
		$command = "$pdflatex -output-directory $ENV{'DOCUMENT_ROOT'}/tmp -interaction batchmode $texFile";
...
sub getImagesFromNewTM {
	...
	my $tempPath = "$ENV{'DOCUMENT_ROOT'}/tmp";
	if (defined($workPath)) {
		$tempPath = $workPath;
	}
	...
	foreach my $image (sort { if ($pdforder eq 'asc') {$a->{'date'} cmp $b->{'date'} } else {$b->{'date'} cmp $a->{'date'}}} @{$images}) {
		...
		if (defined($toConvert{$obj})) {
			...
			$path = sprintf("%s/tmp/%s.tif", $ENV{'DOCUMENT_ROOT'}, $image->{'ObjectId'});
		}
		...
		if ((defined($showTif)) && ($showTif)) {
			...
			if (!-e "$ENV{'DOCUMENT_ROOT'}/tmp/$basefile") {
				symlink($path,"$ENV{'DOCUMENT_ROOT'}/tmp/$basefile");
			}
