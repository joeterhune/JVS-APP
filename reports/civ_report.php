...
function doReport ($caselist) {
    ...
    foreach ($divList as $div => $casetype) {
        ...
        // Build files to be consumed by report graphs
        $rptTypes = array('pend','ro');
        
        foreach ($rptTypes as $rptType) {
            $rpts = array();
            
            if (preg_match('/Circuit/', $rptTitle)) {
                $varName = sprintf("%s_0_120", $rptType);
                $temp = array(
                              'type' => "$rptType" . "_0-120",
                              'count' => count($$varName),
                              'TitleAppend' => '0-120 Days'
                              );
                array_push($rpts, $temp);
                
                $varName = sprintf("%s_121_180", $rptType);
                $temp = array(
            ...
            $reportData =  array(
                array('label' => '0 - 120', 'value' => $rpts[0]['count'],
                      'link' => sprintf("JavaScript:showDivRpt('%s','%s','civ','%s')", $div, $yearMonth, $rpts[0]['type'])),
                array('label' => '121 - 180', 'value' => $rpts[1]['count'],
                      'link' => sprintf("JavaScript:showDivRpt('%s','%s','civ','%s')", $div, $yearMonth, $rpts[1]['type'])),
                array('label' => '181+', 'value' => $rpts[2]['count'],
                      'link' => sprintf("JavaScript:showDivRpt('%s','%s','civ','%s')", $div, $yearMonth, $rpts[0]['type'])),
            );
            
            $reportFile = sprintf("%s/div%s/%s/%s-rpt.json", $outpath, $div, $yearMonth, $rptType);
            
            writeJsonFile($reportData, $reportFile);
        }
    }
}
