...
my %critdesc=(
    ...
    "cldep" => "Closed Cases with Future Events - Dependency",
    "cldel" => "Closed Cases with Future Events - Delinquency"
);
...
my $reopencodes=",Reopen,Reopen VOP,";
...
sub makelist {
    ...
    foreach my $case (keys %caselist) {
        ...
        my($div,$desc,$status,$casetype,$filedate,$dispodate,$reopendate,$reopenclosedate,$ctyp)=split '~',$caselist{$case};
...
#        my $numnopend=makelist($casetype,$div,"nopend");
		...
		my $numcldel = makelist($casetype,$div,"cldel");
...
Closed Cases with Future Events - Delinquency~$numcldel~1~cldel
BLANK
Outstanding Warrants~$numwarrdel~1~warrdel
EOS
...
sub buildcaselist {
	...
	foreach my $casenum (sort keys %rawcase) {
		...
		$caselist{$casenum}="$div~$desc~$status~$casetype~$filedate~$dispodate~$reopendate~$reopenclosedate~$ctyp";
...
sub buildstyles() {
    ...
    if ($DEBUG) {
		...
		foreach my $case (keys %caselist) {
			my ($div, $style, $name, $status, $type, $filedate, $dispodate, $reopendate, $reopenclosedate, $courttype) = split(/~/, $caselist{$case});
