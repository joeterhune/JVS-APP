...
sub doit {
    ...
    my $TMPASS = $conf->{'BenchMark'}->{'defaultPassword'};
    my $TMUSER = $conf->{'BenchMark'}->{'defaultUser'};
    ...
    if ((defined($isShowcase)) && ($isShowcase)) {
        ...
        $workpath = sprintf("/var/jvs/public_html/casefiles/%s/", $ucn);
        if (!-d $workpath) {
            makePaths($workpath);
        }
        ...
        if (inGroup($user,$sealedGroup,$ldap)) {
            ...
            if ($canView) {
                ...
                $TMPASS = $conf->{'BenchMark'}->{'defaultPassword'};
                $TMUSER = $conf->{'BenchMark'}->{'defaultUser'};
            }
	...
	if (scalar(@documents) > 1) {
		$finalPdf = buildImageFile($pdfListFile, \@images, \@documents, $casenum, undef, "/var/jvs/public_html/pdfs");
        $finalPdf =~ s/\var\/jvs\/public_html//g;
	} elsif (scalar(@documents) == 1) {
		# Just a single file - don't do the GhostScript stuff
		my $oldFile = $documents[0]->{'file'};
		my $newFile = sprintf("/var/www/html/pdfs/%s", basename($oldFile));
