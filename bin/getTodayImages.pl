...
foreach my $casenum (@caseList) {
    ...
    if ($pid) {
        ...
        if ($casenum =~ /58-(\d\d\d\d)-(\D\D)-(\d\d\d\d\d\d)-(\D\D\D\D)-(\D\D)/) {
		...
		getDockets($casenum, "/var/www/html/casefiles");
        exit;
    }
}
...
sub getDockets {
    ...
    my $xml;
    eval {
		my $command = "/usr/bin/php $ENV{'JVS_PERL5LIB'}/getTmImages.php -c $caseNum";
		$xml = `$command`;
    };
